...
export default function OrderDetail() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      if (authUser) {
        setUser(authUser);
        supabase.from('customers').select('id').eq('auth_id', authUser.id).single().then(({ data }) => {
        ...
        <div className="bg-white px-4 py-5 shadow-sm border-y border-gray-100">
          ...
          <div className="space-y-4">
            {(() => {
              ...
              // Coba gunakan order.total_price dari Courier app, kalau tidak ada pakai order.total_amount
              const currentTotal = parseFloat(order.total_price) || parseFloat(order.total_amount) || 0;
              const inferredCustomTotal = Math.max(0, currentTotal - totalNonCustom);
              const customItemsCount = items.filter(item => isItemCustom(item)).length;
              const inferredPricePerCustomItem = customItemsCount > 0 ? (inferredCustomTotal / customItemsCount) : 0;
                ...
                <div key={group.id !== 'unknown' ? group.id : groupIdx} className="mb-6 last:mb-0">
                  ...
                  <div className="space-y-4">
                    {group.items.map((item, idx) => {
                      ...
                      // Jika isCustom dan harga di JSON 0, kita pakai harga inferred (jika kurir sudah update total_price > totalNonCustom)
                      const displayPrice = (isCustom && (!item.price || item.price === 0) && inferredPricePerCustomItem > 0) 
                        ? inferredPricePerCustomItem 
                            ...
                            <div className="flex justify-between items-start">
                              <div className="pr-3">
                                ...
                                {item.selectedVariants && Object.keys(item.selectedVariants).length > 0 && (
                                  <p className="text-[10px] text-gray-500 mt-0.5">Varian: {Object.values(item.selectedVariants).join(', ')}</p>
                                )}
                                {!isJasaOnly && (
        ...
        <div className="bg-white mx-4 rounded-2xl p-4 shadow-sm border border-gray-100">
          ...
          <div className="space-y-2 mb-3 pb-3 border-b border-gray-50">
            {!isJasaOnly && (
              <div className="flex justify-between text-sm text-gray-600">
                ...
                <span className="font-semibold">{(order.total_price || order.total_amount) ? `Rp ${(order.total_price || order.total_amount).toLocaleString('id-ID')}` : 'Rp 0'}</span>
              </div>
            )}
            <div className="flex justify-between text-sm text-gray-600">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="font-bold text-primary text-lg">
              {(((order.total_price || order.total_amount) || 0) + (order.delivery_fee || 0)) > 0 
                ? `Rp ${(((order.total_price || order.total_amount) || 0) + (order.delivery_fee || 0)).toLocaleString('id-ID')}` 
